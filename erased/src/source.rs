...
mod private {
    ...
    pub trait DispatchSourceRef<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized = V>:
        DispatchSource<'a>
    {
    ...
    pub trait EraseSourceRef<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized> {
        fn erase_value_source(
            &mut self,
        ) -> crate::private::Erased<&mut dyn DispatchSourceRef<'a, V, R>>;
    }
}
...
pub trait SourceRef<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized = V>:
    Source<'a> + private::EraseSourceRef<'a, V, R>
{
}
...
impl<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized, S: sval::SourceRef<'a, V, R>>
    SourceRef<'a, V, R> for S
{
}

impl<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized, S: sval::SourceRef<'a, V, R>>
    private::EraseSourceRef<'a, V, R> for S
{
    fn erase_value_source(
        &mut self,
    ) -> crate::private::Erased<&mut dyn private::DispatchSourceRef<'a, V, R>> {
        crate::private::Erased(self)
    }
}

impl<'a, V: sval::Value + ?Sized, R: sval::Value + ?Sized, S: sval::SourceRef<'a, V, R>>
    private::DispatchSourceRef<'a, V, R> for S
{
...
impl_value_source!(
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::Source<'a> for dyn SourceRef<'a, V, U> + 'd
    }
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::SourceRef<'a, V, U> for dyn SourceRef<'a, V, U> + 'd
    }
);
impl_value_source!(
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::Source<'a> for dyn SourceRef<'a, V, U> + Send + 'd
    }
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::SourceRef<'a, V, U> for dyn SourceRef<'a, V, U> + Send + 'd
    }
);
impl_value_source!(
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::Source<'a> for dyn SourceRef<'a, V, U> + Send + Sync + 'd
    }
    {
        impl<'a, 'd, V: sval::Value + ?Sized, U: sval::Value + ?Sized> sval::SourceRef<'a, V, U> for dyn SourceRef<'a, V, U> + Send + Sync + 'd
    }
);
