...
pub(crate) fn derive(input: DeriveInput) -> TokenStream {
    ...
    TokenStream::from(quote! {
        ...
        const _: () = {
            ...
            impl sval_generic_api_coroutine::coroutine::CoroutineValue for #ident {
            ...
            pub enum #coroutine_state_field_ident<'a> {
                #(
                    #coroutine_state_field_variant(sval_generic_api_coroutine::coroutine::Slot<<#field_ty as sval_generic_api_coroutine::coroutine::CoroutineValue>::State<'a>>),
                )*
            }

            impl<'a> #coroutine_state_ident<'a> {
                #(
                    fn #field_enter(self: std::pin::Pin<&mut Self>) -> std::pin::Pin<&mut sval_generic_api_coroutine::coroutine::Slot<<#field_ty as sval_generic_api_coroutine::coroutine::CoroutineValue>::State<'a>>> {
                        let self_mut = unsafe { self.get_unchecked_mut() };

                        self_mut.field = Some(#coroutine_state_field_ident::#coroutine_state_field_variant(sval_generic_api_coroutine::coroutine::Slot::new(
                            sval_generic_api_coroutine::coroutine::CoroutineValue::state(&self_mut.value.#field_ident),
                        )));
            ...
            impl<'a, R: sval_generic_api_coroutine::Receiver<'a>> sval_generic_api_coroutine::coroutine::Coroutine<'a, R> for #coroutine_begin_ident {
                type State = #coroutine_state_ident<'a>;

                const MAY_YIELD: bool = true;

                fn resume<'resume>(mut cx: sval_generic_api_coroutine::coroutine::Context<'resume, R, Self>) -> sval_generic_api_coroutine::Result<sval_generic_api_coroutine::coroutine::Resume<'resume, Self>> {
            ...
            #(
                ...
                impl<'a, R: sval_generic_api_coroutine::Receiver<'a>> sval_generic_api_coroutine::coroutine::Coroutine<'a, R> for #coroutine_field_ident {
                    type State = #coroutine_state_ident<'a>;

                    fn resume<'resume>(mut cx: sval_generic_api_coroutine::coroutine::Context<'resume, R, Self>) -> sval_generic_api_coroutine::Result<sval_generic_api_coroutine::coroutine::Resume<'resume, Self>> {
                        let (receiver, state) = cx.state();

                        if !<<#field_ty as sval_generic_api_coroutine::coroutine::CoroutineValue>::Coroutine<'a, R> as sval_generic_api_coroutine::coroutine::Coroutine<'a, R>>::MAY_YIELD {
                            receiver.map_field_entry(#field_str, sval_generic_api_coroutine::coroutine::CoroutineValue::as_value(&state.value.#field_ident))?;
                            ...
                            cx.yield_into_resume::<<#field_ty as sval_generic_api_coroutine::coroutine::CoroutineValue>::Coroutine<'a, R>, #transition_field>(|state| {
                                state.#field_enter()
                            })
                        }
                    }
            ...
            impl<'a, R: sval_generic_api_coroutine::Receiver<'a>> sval_generic_api_coroutine::coroutine::Coroutine<'a, R> for #coroutine_end_ident {
                type State = #coroutine_state_ident<'a>;
