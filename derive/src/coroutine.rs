...
pub(crate) fn derive(input: DeriveInput) -> TokenStream {
    ...
    TokenStream::from(quote! {
        ...
        const _: () = {
            extern crate sval_coroutine;

            impl sval_coroutine::value::CoroutineValue for #ident {
                type State<'a> = #coroutine_state_ident<'a>;
                type Coroutine<'a, R: sval_coroutine::Receiver<'a>> = #coroutine_begin_ident;
            ...
            pub enum #coroutine_state_field_ident<'a> {
                #(
                    #coroutine_state_field_variant(sval_coroutine::co::Coroutine<<#field_ty as sval_coroutine::value::CoroutineValue>::State<'a>>),
                )*
            }

            impl<'a> #coroutine_state_ident<'a> {
                #(
                    fn #field_enter(self: std::pin::Pin<&mut Self>) -> std::pin::Pin<&mut sval_coroutine::co::Coroutine<<#field_ty as sval_coroutine::value::CoroutineValue>::State<'a>>> {
                        let self_mut = unsafe { self.get_unchecked_mut() };

                        self_mut.field = Some(#coroutine_state_field_ident::#coroutine_state_field_variant(sval_coroutine::co::Coroutine::new(
                            sval_coroutine::value::CoroutineValue::state(&self_mut.value.#field_ident),
                        )));
            ...
            impl<'a, R: sval_coroutine::Receiver<'a>> sval_coroutine::co::Resume<'a, R> for #coroutine_begin_ident {
            ...
            #(
                ...
                impl<'a, R: sval_coroutine::Receiver<'a>> sval_coroutine::co::Resume<'a, R> for #coroutine_field_ident {
                    type State = #coroutine_state_ident<'a>;

                    fn resume(mut cx: sval_coroutine::co::Context<Self, R>) -> sval_coroutine::Result<sval_coroutine::co::Yield<Self>> {
                        let (receiver, state) = cx.state();

                        if !<<#field_ty as sval_coroutine::value::CoroutineValue>::Coroutine<'a, R> as sval_coroutine::co::Resume<'a, R>>::MAY_YIELD {
                            ...
                            cx.yield_into_resume::<<#field_ty as sval_coroutine::value::CoroutineValue>::Coroutine<'a, R>, #transition_field>(|state| {
                                state.#field_enter()
                            })
                        }
                    }
            ...
            impl<'a, R: sval_coroutine::Receiver<'a>> sval_coroutine::co::Resume<'a, R> for #coroutine_end_ident {
                type State = #coroutine_state_ident<'a>;
