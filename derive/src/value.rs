...
pub(crate) fn derive(input: DeriveInput) -> TokenStream {
    match &input.data {
        Data::Struct(DataStruct { ref fields, .. }) if fields.len() == 0 => {
            derive_unit_struct(&input.ident, &input.generics)
        }
...
fn derive_struct<'a>(ident: &Ident, generics: &Generics, fields: &FieldsNamed) -> TokenStream {
    ...
    let match_arm = stream_record(quote!(#ident), &ident, None, fields);
...
fn derive_unit_struct<'a>(ident: &Ident, generics: &Generics) -> TokenStream {
    let (impl_generics, ty_generics, _) = generics.split_for_impl();

    let bound = parse_quote!(sval::Value);
    let bounded_where_clause = bound::where_clause_with_bound(&generics, bound);

    let match_arm = stream_tag(quote!(#ident), &ident, None);
